...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    */
    ...
    public function render_login_signup_form($form) {
        global $SITE; 
       // $context['vrtuz_menu']=$this->vrtuz_menu();
...
//***  Footer Logo ******//
    ...
    public function get_footer_logo() {
        ...
        if ($this->should_display_theme_footer_logo()) {
            return $this->get_theme_footer_logo_url();
        }

        $url = $this->get_footer_logo_url();
        if ($url) {
            return $url->out(false);
        }
     ...
     public function should_display_theme_footer_logo() {
            $logo = $this->get_theme_footer_logo_url();

            return !empty($logo);
        }


     public function get_theme_footer_logo_url() {
...
//***  Footer Logo ******//
    ...
    public function vrtuz_menu() {
        ...
        $list .= '<div class="dropdown nav-item"><a class="dropdown-toggle nav-link" id="drop-down-60753a00a9cb660753a005783e337" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" href="/local/ s/index.php">COURSE</a>
    <div class="dropdown-menu" aria-labelledby="drop-down-60753a00a9cb660753a005783e337">';

        if (!empty($themesettings->coursecatedata())) {
            foreach ($themesettings->coursecatedata() as $key => $value) {
                $value = (object)$value;
                $list .= '<a class="dropdown-item" href="/local/courses/index.php?categoryid='.$value->id.'">'.$value->name.'</a>';
            }
        }
        $list .= '</div></div>';
    ...
    public function user_menu($user = null, $withlinks = null) {
        ...
        if (!isloggedin()) {
            $returnstr = '';
            if (!$loginpage) {
                $returnstr .= "<a class='btn btn-login-top btl' href=\"$loginurl\">" . get_string('login') . '</a>';
            }
